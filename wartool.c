...
//      02111-1307, USA.
//
//      $Id: wartool.c,v 1.131 2004/06/27 10:28:46 mr-russ Exp $
...
void ConvertPud(char* file, int pude)
{
	...
	sprintf(buf, "%s/%s.pud.gz", Dir, file);
	CheckPath(buf);
	gf = gzopen(buf, "wb9");
	if (!gf) {
		perror("");
...
int ConvertWav(char* file, int wave)
{
	...
	sc1 = (uint8_t)wavp[16] + ((uint8_t)wavp[17]<<8) + ((uint8_t)wavp[18]<<16) + ((uint8_t)wavp[19]<<24);

	wavp[sc1+20+4] &= 0xfe;	//ensure even length

	sc2 = (uint8_t)wavp[sc1+24] + ((uint8_t)wavp[sc1+25]<<8) + ((uint8_t)wavp[sc1+26]<<16) + ((uint8_t)wavp[sc1+27]<<24);
...
/*	if(sc1!=16) {
		printf("Wav %s, l %d, newl %d, sc1 %d, sc2 %d\n",file,l,newl,sc1,sc2);
	}*/
...
int main(int argc, char** argv)
{
	...
	printf("Wartool Amiga WarpOS version 1.1\n\n");
...
#else
	printf("Please be patient, the data may take several minutes to extract...\n");
#endif
